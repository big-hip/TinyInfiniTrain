...
template <typename... Args> std::string JoinWithDot(const char *, const Args &...parts) {
    std::string result;
    const auto append = [&result](const auto &part) {
        if (!result.empty()) {
            result.push_back('.');
        }
        result += part;
...
std::unique_ptr<GPT2> GPT2::FromLLMC(const std::string &filepath) {
    ...
    auto &transformer_wte_weight = state_dict[JoinWithDot("{}.{}.{}", GPT2::kTransformerLayerName, GPT2::kWTELayerName,
    ...
    auto &transformer_wpe_weight = state_dict[JoinWithDot("{}.{}.{}", GPT2::kTransformerLayerName, GPT2::kWPELayerName,
                                                          nn::Embedding::kParamWeightName)];
    ...
    for (int idx = 0; idx < n_layer; idx++) {
        auto &tensor
            = state_dict[JoinWithDot("{}.{}.{}.{}.{}", GPT2::kTransformerLayerName, GPT2::kHLayerName,
    ...
    for (int idx = 0; idx < n_layer; idx++) {
        auto &tensor = state_dict[JoinWithDot("{}.{}.{}.{}.{}.{}", GPT2::kTransformerLayerName, GPT2::kHLayerName,
    ...
    auto &transformer_ln_f_weight = state_dict[JoinWithDot("{}.{}.{}", GPT2::kTransformerLayerName, GPT2::kLnFLayerName,
    ...
    auto &transformer_ln_f_bias = state_dict[JoinWithDot("{}.{}.{}", GPT2::kTransformerLayerName, GPT2::kLnFLayerName,
